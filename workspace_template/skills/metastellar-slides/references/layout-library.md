# Metastellar Slide Layout Library
...
- **Title**: top of slide, Primary Dark (blue-800) Segoe UI. Right-aligned for most content slides;
  centered/left also seen. One or two lines.
- **Corner accent**: thin Primary Dark (blue-800) block at the top-right edge (from template).
- **Logo**: Logo bottom-left. **Slide number** bottom-right. (template footer)
...
- **Message bar**: full-width **Primary Dark (blue-800)** band near the top, white 20 pt Segoe UI bold —
  the core statement.
- **Yellow callout**: a benefit sentence, Primary Dark (blue-800) text.
- **Gray bullet box** (`#F3F4F6`, slight transparency): the supporting bullet list, dark-blue
  bullets.
...
- **Feature boxes** (2–3): each a filled rectangle with a bold heading + 1–2 short paragraphs.
  Color them from the dark-blue ramp + one gray box (e.g. box 1 = `#1E40AF`, box 2 = `#2563EB`,
  side box = `#F3F4F6`). White text on the blue boxes, dark-blue text on gray.
...
- **Two equal columns.** Each column:
  - **Header bar**: product name (bold) + one-line descriptor. Differentiate the two with
    **Primary Dark (blue-800)** (left) and **Primary (blue-500)** (right) headers (white text).
  - **Photo** of the application.
  - **Gray box** (`#F3F4F6`) with 2–3 spec bullets, plus a small product (package) shot.
...
- **Header bar** options:
  - **Yellow** header, Primary Dark (blue-800) text (clean, high-energy) — see High-power example.
...
- **Gray box** (`#F3F4F6`) holds the bullets/short text (centered text works for 1–3 lines).
...
Same as `cards-Nup` (4 graded-header cards) **plus** a **full-width Accent (indigo-500) message bar**
across the bottom with the single takeaway (e.g. the part number + "Efficient, flexible, and
available!"), Primary Dark (blue-800) text, bold key words. Use when the cards build to one conclusion.

## 6. `icon-cards-5up` — categories with icons
- **Top message bar**: Primary Dark (blue-800) full-width band, white text — the umbrella statement.
- **5 columns** (4–6 works), each:
  - **Yellow icon tile** (square) with a dark-blue **monochrome icon** (SVG pictogram, not
    SmartArt, not AI art).
  - **Gray box** (`#F3F4F6`) with a bold heading, a standard/label line, and a short
    description — all Primary Dark (blue-800), centered.
- Ideal for certifications, pillars, capability families.

## 7. `quadrant-2x2-center-badge` — four facets, visual
- **2×2 grid.** Cells alternate **photo** and **colored text panel**; panels use `#1E40AF`
  and `#2563EB` (white headings + short body).
...
- **6 tiles** in 2 rows × 3 columns (also works 1×N or 2×2). Each tile = **caption header bar +
  photo**.
- **Alternate header colors** tile-to-tile between **Primary Dark (blue-800)** (white text) and
  **Accent (indigo-500)** (dark-blue text) for rhythm.
- Use for "our product families", "our markets", a visual table of contents.

## 9. `process-flow-circles` — sequential steps
- **N circles** in a row (5 shown), **alternating `#1E40AF` and `#8091A5`**, white text inside
  (short label per step).
- **Dotted connector** weaving through the circles; an arrowhead on the last.
- **Step markers**: small **Accent (indigo-500)** dots on short lines above/below alternating circles,
  each with "**Step N**" (bold) + a label, Primary Dark (blue-800).
- Use for workflows/pipelines. Don't exceed ~6 steps.

## 10. `row-label-table-visual` — labelled rows pointing to a visual
- **Left stack of rows**: each row = **Primary Dark (blue-800) label box** (white bold) + **gray content
  cell** (`#F3F4F6`, holds a logo/diagram/text) + optional right-side annotation text.
- **Large gray arrow** pointing right from the rows to a **supporting visual** (e.g. a product
  screenshot / device render).
- **Light-blue URL button** (`#3B82F6`) bottom with a globe pictogram for the call-to-action link.
- Use to map "capability → asset → where it lives".

## 11. `timeline-era-cards` — evolution over time
- **N era cards** across the top: each = **Accent (indigo-500) header** (era name, dark-blue bold) +
  **gray box** (`#F3F4F6`) with bullets.
- **Navy arrow timeline** beneath them (`#1E40AF`, arrowhead at right) with **Accent (indigo-500) circle
...
1. **Organic lane** — activity boxes above the axis, each linked to a date marker by a thin
   Primary Dark (blue-800) vertical connector. Box = Primary Dark (blue-800) header (white bold) + gray `#F3F4F6`
   body (dark-blue bullets). One box per milestone; larger box for major launch events.
2. **Central timeline axis** — thick Primary Dark (blue-800) right-pointing arrow spanning the slide;
   **yellow circle** markers on the axis at each date; date labels below markers (Segoe UI 12–14 pt,
   Primary Dark (blue-800)). Use **double vertical white break lines** when the timeline skips months
   (e.g. Jan–Mar → December).
3. **Paid lane** — Primary (blue-500) horizontal bar(s) below the axis showing campaign duration
   spans (e.g. "Paid Social 11-Feb→10-Mar"); light-blue caption for always-on channels below.
4. **Product-focus bar** — bottom segmented Primary (blue-500) arrow; each segment = one product
   focus period; separate segments with double vertical break lines.

**Colors:** Primary Dark (blue-800) (axis, connectors, organic headers), Accent (indigo-500) (date markers),
Primary (blue-500) (paid bars, product-focus arrow), Gray 1 (box bodies). Max 3 colors per region.
...
1. **Legend row** (top-right or below title) — small swatches:
   - Accent (indigo-500) fill = highlighted track item (e.g. Cube2-related)
...
3. **Central timeline axis** — Primary Dark (blue-800) horizontal line with month/date tick labels
...
4. **Promotion lane** — event items below axis: icon + label per activity (blog, banner, webinar,
   ...
   (Primary (blue-500) or approved accent) spanning start→end months (e.g. Google + Social ads
   Apr→Jun). Yellow-background items = same highlight as legend.

**Lane labels:** "CONTENT ASSETS" and "PROMOTION" in Primary Dark (blue-800) bold, left-aligned above
each lane.

**Colors:** Primary Dark (blue-800) (axis, labels), Accent (indigo-500) (highlight boxes/items), Gray 1–2 (TBC items),
Primary (blue-500) (campaign duration bars). Avoid magenta/green except the single MML marker.
...
**Structure:**
1. **Left hero image** — full-height panel (~1/3 slide width). Logo bottom-left on the image.
   Pass `img_path` for a real ST-owned photo; omit for a gray **placeholder** labelled "Image".
2. **Title** — top-right on the white band, Primary Dark (blue-800) Segoe UI 24–28 pt.
3. **Rows** (repeat 3–5×) — **Accent (indigo-500)** square tile (optional `icon_path` pictogram centred
   inside) + **Gray 1** `#F3F4F6` bar with one dark-blue sentence (bold key phrases inline).
4. **Punchline** — large dark-blue statement bottom-right (optional).

**Colors:** Primary Dark (blue-800), Gray 1, Accent (indigo-500) accent. No standard message bar on this archetype.
...
**Structure:**
1. **Title + subtitle** — top-right, Primary Dark (blue-800).
2. **Dotted baseline** — horizontal dashed connector across the slide.
3. **Circles** (3–6) — alternating Primary Dark (blue-800) and slate ramp fill; white/yellow label text
   centred inside each circle.
4. **Yellow callout ticks** — short vertical Accent (indigo-500) bar above or below each circle, with a
   caption text block (10–11 pt dark blue).
5. **Logo** — bottom-left.

**Colors:** Primary Dark (blue-800), slate ramp, Accent (indigo-500) markers. Distinct from `process-flow-circles`
...
**Structure:**
1. **Title** — top-right, Primary Dark (blue-800).
2. **Navy message bar** — full-width Primary Dark (blue-800) band overlapping the top of the left image;
   white 16–18 pt bold statement inside.
3. **Left hero image** — ~40% width below the bar; logo bottom-left. `img_path` or gray
   placeholder.
4. **Category rows** (2–4) — **Accent (indigo-500)** narrow header box (category name) + **Gray 1**
...
3. Respect the **2–3 color** rule per slide — these archetypes already do (blue ramp + gray +
   one yellow accent).
4. Source photos from user assets; icons as **SVG pictograms**; **no AI imagery, no SmartArt**.
...
`left-image-tiered-list`) are in `pptx-implementation.md` and `slide_theme.py`.
